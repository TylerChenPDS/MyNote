...
先声明几个概念。①seq：序号字段，指的是本报文段所发送数据的第一个字节的序号。②ack：确认字号段，指的是期望收到对方下一个报文段的第一个数据字节的序号。③、SYN为1和FIN为1 的报文不能携带数据，但还是要消耗一个seq 

**1）建立连接的3次握手**

a)   客户端向处于LISTEN状态的服务器发送请求连接报文首部为SYN=1,seq=x
...
b)   服务器ACK=1，seq=y，ack=x+1 此时A到B方向的连接就释放了，此时TCP连接处于半连接状态，即A已经没有数据要发送了，但是B如果有数据发送A仍然要接收。

c)   服务器B数据发送完了向A发送：FIN=1，ACK=1，seq=z，ack=x+1

d)   A发送 ACK=1，seq=x+1，ack=z+1 注意，此时tcp连接并没有释放，必须经过时间等待计时器**2MST（最长报文段寿命时间 Maximum Segment Lifetime）**时间后才能释放。
...
![image-20210219165337310](C:%5CUsers%5C19699%5CAppData%5CRoaming%5CTypora%5Ctypora-user-images%5Cimage-20210219165337310.png)
